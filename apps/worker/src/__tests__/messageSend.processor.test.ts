...
import { CircuitBreaker, InMemoryBreakerStore, InMemoryRateStore, RateLimiter } from '@wise/queue';
...
describe.skipIf(!hasDb)('processMessageSend', () => {
  ...
  it('rate limiter esgotado adia o 2º envio no mesmo número (retryable, §41)', async () => {
    const server = new MetaMockServer();
    // Duas mensagens no MESMO número (mesma cadeia).
    const chain = await seedCampaignChain(prisma, vault);
    const campaign = await seedCampaign(prisma, chain.org.id, chain.template.id, chain.account.id);
    const messages = [];
    for (let i = 0; i < 2; i += 1) {
      const contact = await prisma.contact.create({
        data: { organizationId: chain.org.id, phone: `+55119900012${i}${i}` },
      });
      const recipient = await prisma.campaignRecipient.create({
        data: {
          organizationId: chain.org.id,
    ...
    const first = await processMessageSend(prisma, meta, messages[0]!.id, undefined, rl);
    expect(first.status).toBe('SENT');
    // 2º envio no mesmo número: sem tokens → adiado (throw retryable), sem chamar a Meta.
    const callsAfterFirst = server.calls.length;
    await expect(
      processMessageSend(prisma, meta, messages[1]!.id, undefined, rl),
    ).rejects.toThrow();
    expect(server.calls.length).toBe(callsAfterFirst);
  });
